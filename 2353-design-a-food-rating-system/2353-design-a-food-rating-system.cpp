class FoodRatings
{
    ...
    FoodRatings(vector<string> &foods, vector<string> &cuisines, vector< int > &ratings)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            cousin_rating[cuisines[i]].insert({-ratings[i], foods[i]});
    ...
    void changeRating(string food, int newRating)
    {
        ...
        // auto &target = find_cusine[food].second; 
        
        cousin_rating[find_cusine].erase({-curr_rating, food});
        
        cousin_rating[find_cusine].insert({-newRating, food});
